...
ax.set_xticks([])
ax.set_yticks([])
ax.set_title("Kowloon Hourly Weather - Bubble Chart (Real Data)", color='#aeeaff', fontsize=13, pad=10)
...
def draw_panel(frame_idx):
    ...
    ax_panel.text(0.04, 0.95, "Mapping", color='#7fffd4', fontsize=12, weight='bold', va='top')
    ax_panel.text(0.06, 0.88, "Temperature -> Color (light blue -> light green)", color='#cfefff', fontsize=9)
    ax_panel.text(0.06, 0.83, "Humidity -> Bubble size", color='#cfefff', fontsize=9)
    ax_panel.text(0.06, 0.78, "Wind speed -> Bubble count", color='#cfefff', fontsize=9)
    ax_panel.text(0.06, 0.73, "Pressure -> Position (2D)", color='#cfefff', fontsize=9)
    ...
    ax_panel.text(gx0, gy+0.05, "Low", color='#9bd0ff', fontsize=8)
    ax_panel.text(gx1-0.06, gy+0.05, "High", color='#bdf5d5', fontsize=8)
    ...
    ax_panel.text(0.04, 0.60, "Current", color='#7fffd4', fontsize=12, weight='bold')
    ax_panel.text(0.06, 0.54, f"Time: {times[frame_idx].replace('T',' ')}", color='#cfefff', fontsize=9)
    ax_panel.text(0.06, 0.49, f"Temperature: {t:.1f}°C", color='#aeeaff', fontsize=10)
    ax_panel.text(0.06, 0.44, f"Humidity: {h:.0f}%", color='#aeeaff', fontsize=10)
    ax_panel.text(0.06, 0.39, f"Wind speed: {w:.1f} km/h", color='#aeeaff', fontsize=10)
    ax_panel.text(0.06, 0.34, f"Pressure: {p:.0f} hPa", color='#aeeaff', fontsize=10)

    # Sparklines (last 36 hours)
    N = 36
    s0 = max(0, frame_idx-N+1)
    idx = slice(s0, frame_idx+1)
    sparkline(ax_panel, 0.06, 0.26, 0.88, 0.05, temperature_2m[idx], '#bdf5d5', 'Temp')
    sparkline(ax_panel, 0.06, 0.19, 0.88, 0.05, humidity_2m[idx], '#9bd0ff', 'Humidity')
    sparkline(ax_panel, 0.06, 0.12, 0.88, 0.05, wind_10m[idx], '#7fffd4', 'Wind')
    sparkline(ax_panel, 0.06, 0.05, 0.88, 0.05, pressure_sl[idx], '#74c5ff', 'Pressure')

    ax_panel.text(0.04, 0.01, "Source: Open-Meteo (hourly) | Location: Kowloon", color='#6fbadf', fontsize=8)
